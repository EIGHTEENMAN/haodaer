<template>
  <div class="skill-select-overlay" v-if="gameStore.showSkillSelect">
    <div class="skill-select-box">
      <div class="select-title">SELECT SKILL</div>
      <div
      ...
      >
        ...
        <div v-if="skill.cooldown > 0" class="skill-cd">{{ Math.ceil(skill.cooldown / 1000) }}s</div>
        <div v-if="!skill.unlocked" class="skill-locked">🔒 LOCKED</div>
      </div>
      <div class="select-hint">PRESS 1-5 OR CLICK</div>
    </div>
  </div>
</template>
