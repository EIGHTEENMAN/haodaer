<template>
  <div class="start-screen">
    <div class="start-box">
      <h1 class="title">学 英 语</h1>
      <div class="subtitle">打怪学单词 · 好大儿</div>

      <!-- Auth info -->
      <div class="auth-area">
        <div v-if="loggedIn" class="user-info">
          <span class="avatar">{{ userInitial }}</span>
          <div>
            <div class="username">{{ userName }}</div>
            <div class="game-nickname-row">
              游戏昵称: <input v-model="gameNickname" class="nick-input" placeholder="小勇士" @blur="saveNickname" />
            </div>
          </div>
        </div>
        <div v-else class="login-hint">
          <div class="login-icon">🔒</div>
          <p>使用好大儿账号登录后可以保存学习记录</p>
          <button class="btn" @click="goLogin">去登录</button>
        </div>
      </div>

      <div class="stats-summary" v-if="hasRecords">
        <p>已学 {{ wordCount }} 个单词 · 掌握 {{ masteredCount }} 个</p>
      </div>

      <button class="start-btn" @click="$emit('start')">
        开始游戏
      </button>

      <button v-if="hasRecords" class="review-btn" @click="$emit('review')">
        复习单词
      </button>

      <div class="controls-hint">
        <p>WASD / 摇杆 移动 · 1-5 选技能</p>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted, computed } from "vue"
import { wordStore, getMasteredCount } from "../stores/wordStore"

const emit = defineEmits<{ start: [] }>()

const loggedIn = ref(false)
const userName = ref("")
const gameNickname = ref(localStorage.getItem("ultraman_nickname") || "")
const userInitial = ref("")

onMounted(() => {
  // Check auth from main-site system
  const token = localStorage.getItem("haodaer_token")
  if (token) {
    try {
      const userStr = localStorage.getItem("haodaer_user")
      if (userStr) {
        const user = JSON.parse(userStr)
        userName.value = user.nickname || user.username || "用户"
        userInitial.value = userName.value.charAt(0)
        loggedIn.value = true
      }
    } catch {}
  }
})

function saveNickname() {
  localStorage.setItem("ultraman_nickname", gameNickname.value)
}

function goLogin() {
  window.location.href = "https://grandand.com/login?redirect=" + encodeURIComponent(window.location.href)
}

const hasRecords = computed(() => wordStore.records.size > 0)
const wordCount = computed(() => wordStore.records.size)
const masteredCount = computed(() => getMasteredCount())
</script>

<style scoped>
.start-screen {
  position: fixed;
  inset: 0;
  background: #1a1a2e;
  display: flex;
  align-items: center;
  justify-content: center;
  z-index: 900;
}
.start-box {
  text-align: center;
  padding: 40px;
}
.title {
  font-size: 56px;
  color: #ffd700;
  text-shadow: 0 0 20px rgba(255,215,0,0.5), 3px 3px 0 #8b4513;
  margin-bottom: 8px;
  font-family: 'Press Start 2P', monospace;
  letter-spacing: 12px;
}
.subtitle {
  color: #88ccff;
  font-size: 18px;
  margin-bottom: 32px;
  font-family: 'Press Start 2P', monospace;
}
.auth-area {
  background: rgba(255,255,255,0.05);
  border: 2px solid rgba(255,255,255,0.15);
  
  padding: 16px 24px;
  margin-bottom: 20px;
  min-width: 280px;
}
.user-info {
  display: flex;
  align-items: center;
  gap: 12px;
}
.avatar {
  width: 40px;
  height: 40px;
  border-radius: 0;
  background: #d4a017;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 18px;
  color: #1a1a2e;
  font-weight: bold;
}
.username {
  color: #fff;
  font-size: 14px;
  text-align: left;
}
.game-nickname-row {
  color: #aaa;
  font-size: 12px;
  margin-top: 4px;
  display: flex;
  align-items: center;
  gap: 4px;
}
.nick-input {
  background: rgba(255,255,255,0.1);
  border: 1px solid rgba(255,255,255,0.2);
  
  color: #fff;
  padding: 2px 8px;
  font-size: 12px;
  width: 100px;
  font-family: 'Press Start 2P', monospace;
}
.nick-input:focus {
  outline: none;
  border-color: #ffd700;
}
.login-hint {
  color: #888;
  font-size: 13px;
}
.login-hint p { margin-bottom: 8px; }
.login-icon { font-size: 24px; margin-bottom: 8px; }
.btn {
  padding: 8px 20px;
  border: 1px solid #ffd700;
  
  background: transparent;
  color: #ffd700;
  font-family: 'Press Start 2P', monospace;
  cursor: pointer;
  font-size: 13px;
}
.btn:hover { background: rgba(255,215,0,0.15); }
.stats-summary {
  color: #66cc66;
  font-size: 13px;
  margin-bottom: 20px;
}
.start-btn {
  display: block;
  margin: 0 auto 12px;
  padding: 16px 60px;
  border: 3px solid #ffd700;
  
  background: #d4a017;
  color: #1a1a2e;
  font-size: 22px;
  font-family: 'Press Start 2P', monospace;
  font-weight: bold;
  cursor: pointer;
  transition: all 0.1s;
}
.start-btn:hover {
  transform: scale(1.05);
  filter: brightness(1.1);
}
.review-btn {
  display: block;
  margin: 0 auto;
  padding: 10px 40px;
  border: 1px solid #88ccff;
  
  background: transparent;
  color: #88ccff;
  font-size: 14px;
  font-family: 'Press Start 2P', monospace;
  cursor: pointer;
}
.review-btn:hover { background: rgba(136,204,255,0.1); }
.controls-hint {
  margin-top: 24px;
  color: #666;
  font-size: 12px;
}
</style>
